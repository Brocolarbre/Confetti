...
{
	factory.registerType("ColorShift", [](const json& data) { return std::make_unique<cft::ColorShiftVisualBehavior>(JsonTraits<cft::Color, Color>::readVector(data.at("colors")), data.at("speed"), data.at("cyclic")); });
	...
	factory.registerType("Pulse", [](const json& data) { return std::make_unique<cft::PulseVisualBehavior>(data.at("colorA").get<Color>().value, data.at("colorB").get<Color>().value, data.at("speed")); });
	...
	factory.registerType("SmoothColorShift", [](const json& data) { return std::make_unique<cft::SmoothColorShiftVisualBehavior>(JsonTraits<cft::Color, Color>::readVector(data.at("colors")), data.at("speed"), data.at("cyclic")); });
	factory.registerType("SquashStretch", [](const json& data) { return std::make_unique<cft::SquashStretchVisualBehavior>(data.at("strength").get<Vec2>().value, data.at("speed")); });
}