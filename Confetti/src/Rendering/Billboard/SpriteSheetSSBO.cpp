#include "Confetti/Rendering/Billboard/SpriteSheetSSBO.hpp"
...
namespace cft
{
	...
	void SpriteSheetSSBO::setData(const std::vector<std::reference_wrapper<const SpriteSheetDescriptor>>& spriteSheetData)
	{
		...
		for (const SpriteSheetDescriptor& spriteSheetDescriptor : spriteSheetData)
		{
			spriteSheetsData.push_back(SpriteSheetData{
				glm::uvec2(spriteSheetDescriptor.frameWidth, spriteSheetDescriptor.frameHeight),
				glm::max(spriteSheetDescriptor.frameCount, 1u),
				glm::max(spriteSheetDescriptor.rowFrameCount, 1u),
				spriteSheetDescriptor.animationSpeed,
				glm::vec3(0.0f)
			});
		}
