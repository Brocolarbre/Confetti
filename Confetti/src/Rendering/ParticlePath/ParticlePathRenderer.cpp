#include "Confetti/Rendering/ParticlePath/ParticlePathRenderer.hpp"
#include "Confetti/Rendering/ShaderSource/ParticlePathShaderSource.hpp"
...
namespace cft
{
	void ParticlePathRenderer::generateGeometry(const std::deque<PathPoint>& trail, const PathConfiguration& pathConfiguration, const View& view, ParticlePathGeometry& geometry) const
	{
		...
		{
			const PathPoint& previousPoint = pointIndex > 0 ? trail[static_cast<size_t>(pointIndex) - 1] : trail[pointIndex];
			const PathPoint& currentPoint = trail[pointIndex];
			const PathPoint& nextPoint = pointIndex < trail.size() - 1 ? trail[static_cast<size_t>(pointIndex) + 1] : trail[pointIndex];
	...
	ParticlePathRenderer::ParticlePathGeometry ParticlePathRenderer::generateTrailGeometry(const std::unordered_map<unsigned int, TrailPool>& trailPools, const TrailRegistry& trailRegistry, const View& view)
	{
		...
		{
			const std::vector<std::deque<PathPoint>>& trailPoints = pool.getPathPoints();
		...
		for (const auto& [id, pool] : trailPools)
		{
			...
			for (unsigned int i = 0; i < poolCount; ++i)
			{
				const std::deque<PathPoint>& trail = trailPoints[i];
	...
	ParticlePathRenderer::ParticlePathGeometry ParticlePathRenderer::generateRibbonGeometry(const std::unordered_map<unsigned int, RibbonPool>& ribbonPools, const RibbonRegistry& ribbonRegistry, const View& view)
	{
		...
		{
			const std::vector<std::deque<PathPoint>>& ribbonPoints = pool.getRibbonPoints();
			size_t poolCount = pool.getCount();
		...
		for (const auto& [id, pool] : ribbonPools)
		{
			...
			for (unsigned int i = 0; i < poolCount; ++i)
			{
				const std::deque<PathPoint>& trail = ribbonPoints[i];
