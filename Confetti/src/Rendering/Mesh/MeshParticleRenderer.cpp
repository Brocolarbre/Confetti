#include "Confetti/Rendering/Mesh/MeshParticleRenderer.hpp"
	...
	void MeshParticleRenderer::update(const std::unordered_map<unsigned int, ParticlePool>& particlePools, const ParticleRegistry& particleRegistry)
	{
		...
		for (const auto& [id, pool] : particlePools)
		{
			...
			for (unsigned int i = 0; i < count; ++i)
			{
				...
				if (entry.renderConfiguration.renderType != RenderType::Mesh)
					continue;

				const MeshRenderConfiguration& descriptor = std::get<MeshRenderConfiguration>(entry.renderConfiguration.configurationData);
