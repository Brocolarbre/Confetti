...
namespace cft
{
	...
	unsigned int ParticleRegistry::createEntry(std::vector<std::unique_ptr<ForceField>> forceFields, std::vector<std::unique_ptr<MotionBehavior>>& motionBehaviors, std::vector<std::unique_ptr<ParticleBehavior>>& particleBehaviors)
	{
		unsigned int id = m_nextId++;
		m_entries.insert({ id, ParticleRegistryEntry{ 0, std::move(forceFields), std::move(motionBehaviors), std::move(particleBehaviors) }});
