...
namespace cft
{
	SpawnContext CylinderSpawnShape::generateValue(unsigned int count, unsigned int index) const
	{
		...
		glm::vec3 position = m_axis * currentHeight + radial * m_radius;
		glm::vec3 normal = glm::normalize(radial);

		return SpawnContext{ position, normal };
	}

	CylinderSpawnShape::CylinderSpawnShape(float height, float radius, const glm::vec3& axis) :
		m_height(height),
	...
	std::unique_ptr<SpawnShape> CylinderSpawnShape::clone() const
	{
		return std::make_unique<CylinderSpawnShape>(*this);
	}
}