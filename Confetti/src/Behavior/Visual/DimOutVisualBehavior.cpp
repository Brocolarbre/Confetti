#include "Confetti/Behavior/Visual/DimOutVisualBehavior.hpp"

namespace cft
{
	DimOutVisualBehavior::DimOutVisualBehavior(ParticleTime duration) :
		m_duration(duration)
	{

	}

	void DimOutVisualBehavior::evaluate(float elapsedTime, float normalizedAge, ParticleView& particle)
	{
		float duration = m_duration.normalize(particle.lifetime);
		float start = 1.0f - duration;

		if (normalizedAge < start)
			return;

		float t = (normalizedAge - start) / duration;
		float alpha = particle.color.a;
		particle.color = (1.0f - t) * particle.initialColor;
		particle.color.a = alpha;
	}
}