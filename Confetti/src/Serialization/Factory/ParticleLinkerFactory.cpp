...
namespace cft
{
	void JsonLoader::registerTypes(JsonFactory<cft::ParticleLinker>& factory, const ProviderRegistry& providerRegistry)
	{
		...
		factory.registerType("Origin", [](const json& data) { return std::make_unique<cft::OriginParticleLinker>(std::move(data.at("connectionRules").get<std::vector<std::unique_ptr<cft::LinkRule>>>()), std::move(data.at("validationRules").get<std::vector<std::unique_ptr<cft::LinkRule>>>()), data.at("origin").get<JsonTypes::Vec3>().value); });
