...
namespace cft
{
	void JsonLoader::registerTypes(JsonFactory<cft::MotionBehavior>& factory)
	{
		...
		factory.registerType("Orbit", [](const json& data) { return std::make_unique<cft::OrbitMotionBehavior>(data.at("origin").get<JsonTypes::Vec3>().value, data.at("axis").get<JsonTypes::Vec3>().value, data.at("radius"), data.at("speed")); });
